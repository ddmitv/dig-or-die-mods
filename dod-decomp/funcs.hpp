...
inline bool CellHasAlivePlant(const CCell& cell, const CItem_PluginData* const itemsData) {
    return itemsData[cell.m_contentId].m_isPlant != 0 && (cell.m_flags & Flag_CustomData0) == 0;
}
...
// FUNCTION: 0x54d0
inline void PostProcessSimulation(int* const changeCellPos) {
    // changeCellPos encoding: cellX * gsY + cellY + changeType * 10000000
    ...
    int startOffset = 0;
    int numIterations = 0;
    GetIteratorsFreq(g_gridSize.y - 4, g_simuTime, g_simuDeltaTime, 2., startOffset, numIterations);

    for (int iteration = 0; iteration < numIterations; ++iteration) {
        const int y = (iteration + startOffset) % (g_gridSize.y - 4) + 2; // simplified from 2 mod operations -> 1 mod operation
        g_nbCellsUpdated[y] = 10;
        for (int x = 1; x < g_gridSize.x - 1; ++x) {
            const int centerCellIdx = x * g_gridSize.y + y;
            const int leftCellIdx = (x - 1) * g_gridSize.y + y;
            const int rightCellIdx = (x + 1) * g_gridSize.y + y;
            const int bottomCellIdx = centerCellIdx - 1;
            const int topCellIdx = centerCellIdx + 1;

            CCell& centerCell = g_grid[centerCellIdx];
            const CCell& leftCell = g_grid[leftCellIdx];
            const CCell& rightCell = g_grid[rightCellIdx];
            const CCell& bottomCell = g_grid[bottomCellIdx];
            const CCell& topCell = g_grid[topCellIdx];
            const CCell& bottomLeftCell = g_grid[leftCellIdx - 1];
            const CCell& bottomRightCell = g_grid[rightCellIdx - 1];

            if (CellHasFlag(centerCell, Flag_IsMapped)
                && float(x) > g_gridBorderNoCam
                && (float(x) < float(g_gridSize.x) - g_gridBorderNoCam)) {
                g_nbCellsUpdated[y] += 1;
            }
            if (IsCellPassable(centerCell, g_itemsData) && CellHasLava(centerCell) && centerCell.m_water > 0.3 && changedCellCount < 1000) {
                if (centerCell.m_water < 1.5f && double(::rand()) < (double(RAND_MAX) * 0.005)
                    ...
                    && (CellHasBgSurface(centerCell)
                        || (CellHasBgSurface(bottomCell)
                            && CellHasBgSurface(bottomLeftCell)
                            && CellHasBgSurface(bottomRightCell)))
                    && (!IsCellPassableWithLava(bottomCell, g_itemsData)
                        || centerCell.m_water >= 1.0f || AlmostEqual(bottomCell.m_water, centerCell.m_water / 10.f + 1.f))
                    && (!IsCellPassableWithLava(bottomCell, g_itemsData)
                        || centerCell.m_water < 1.0f || AlmostEqual(bottomCell.m_water, centerCell.m_water + 0.1f))
                    && (!IsCellPassableWithLava(topCell, g_itemsData)
                        || centerCell.m_water <= 1.0f || centerCell.m_water >= 1.1f || AlmostEqual(centerCell.m_water, topCell.m_water / 10.f + 1.f))
                    && (!IsCellPassableWithLava(topCell, g_itemsData)
                        || centerCell.m_water < 1.1f || AlmostEqual(centerCell.m_water, topCell.m_water + 0.1f))
                    && (!IsCellPassableWithLava(leftCell, g_itemsData)
                        || AlmostEqual(centerCell.m_water, leftCell.m_water))
                    && (!IsCellPassableWithLava(rightCell, g_itemsData)
                        || AlmostEqual(centerCell.m_water, rightCell.m_water))
                    )
                {
                    changeCellPos[changedCellCount] = centerCellIdx + changeCellType::basaltFormation;
                    changedCellCount += 1;
                }
                if (centerCell.m_water > 2.f) {
                    const double basaltMeltChance = std::min(double(centerCell.m_water / 10.f) + 0.2, 1.0);
                    if (double(::rand()) < basaltMeltChance * double(RAND_MAX)) {
                        if (g_itemsData[bottomCell.m_contentId].m_isBasalt != 0 && changedCellCount < 1000) {
                            changeCellPos[changedCellCount] = bottomCellIdx + changeCellType::basaltMelt;
                            changedCellCount += 1;
                        }
                        if (g_itemsData[topCell.m_contentId].m_isBasalt != 0 && changedCellCount < 1000) {
                            changeCellPos[changedCellCount] = topCellIdx + changeCellType::basaltMelt;
                            changedCellCount += 1;
                        }
                        if (g_itemsData[leftCell.m_contentId].m_isBasalt != 0 && changedCellCount < 1000) {
                            changeCellPos[changedCellCount] = leftCellIdx + changeCellType::basaltMelt;
                            changedCellCount += 1;
                        }
                        if (g_itemsData[rightCell.m_contentId].m_isBasalt != 0 && changedCellCount < 1000) {
                            changeCellPos[changedCellCount] = rightCellIdx + changeCellType::basaltMelt;
                            changedCellCount += 1;
                        }
                    }
                }
                if (centerCell.m_water > 0.3f && double(::rand()) < double(RAND_MAX) * 0.2) {
                    for (int i = 0; i < 5; ++i) {
                        const int cellIdx = (g_lavaInteractionDx[i] + x) * g_gridSize.y + (g_lavaInteractionDy[i] + y);
                        if (g_itemsData[g_grid[cellIdx].m_contentId].m_isDirt != 0
                            && changedCellCount < 1000
                            && (centerCell.m_water > 3.f || double(::rand()) < double(RAND_MAX) * 0.3)) {
                            changeCellPos[changedCellCount] = cellIdx + changeCellType::basaltFormation;
                            changedCellCount += 1;
                        }
                    }
                }
            }
            if (IsCellPassable(centerCell, g_itemsData) && CellHasFlag(centerCell, Flag_IsLava) && centerCell.m_water > 0.001f) {
                for (int i = 0; i < 5; ++i) {
                    CCell& dirtCell = g_grid[(g_lavaInteractionDx[i] + x) * g_gridSize.y + (g_lavaInteractionDy[i] + y)];
                    if ((dirtCell.m_contentId != 0
                        && g_itemsData[dirtCell.m_contentId].m_isMineral == 0
                        && g_itemsData[dirtCell.m_contentId].m_isFireProof == 0
                        ) || CellHasGrass(dirtCell, g_itemsData)) {
                        CellSetFlag(dirtCell, Flag_IsBurning, true);
                    }
                }
            }
            if (CellHasFlag(centerCell, Flag_IsBurning)) {
                if (centerCell.m_contentId != 0 && (
                       (IsCellPassable(centerCell, g_itemsData) && CellHasFlag(centerCell, Flag_IsLava))
                    || ((!IsCellPassable(centerCell, g_itemsData) || centerCell.m_water <= 0.002f)
                        && (!CellHasGrass(centerCell, g_itemsData) || topCell.m_water <= 0.002f)))) {
                    if (changedCellCount < 1000) {
                        changeCellPos[changedCellCount] = centerCellIdx + changeCellType::damageCell;
                        changedCellCount += 1;
                        for (int i = 0; i < 12; ++i) {
                            CCell& currentCell = g_grid[(g_fireSpreadDx[i] + x) * g_gridSize.y + (g_fireSpreadDy[i] + y)];
                            if ((CellHasAlivePlant(currentCell, g_itemsData) &&
                                ...
                                CellSetFlag(currentCell, Flag_IsBurning, true);
                            }
                        }
                    }
                } else {
                    CellSetFlag(centerCell, Flag_IsBurning, false);
                }
            }
        }
    }
